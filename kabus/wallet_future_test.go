...
func Test_restClient_WalletFuture(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			mux.HandleFunc("/wallet/future", func(w http.ResponseWriter, _ *http.Request) {
...
func Test_restClient_WalletFutureSymbol(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			mux := http.NewServeMux()
			mux.HandleFunc("/wallet/future/165120018@2", func(w http.ResponseWriter, _ *http.Request) {
				w.WriteHeader(test.status)
				_, _ = w.Write([]byte(test.body))
			})
			ts := httptest.NewServer(mux)
			...
			req := &restClient{url: ts.URL}
			got1, got2 := req.WalletFutureSymbol("", WalletFutureSymbolRequest{Symbol: "165120018", Exchange: FutureExchangeAll})
			if !reflect.DeepEqual(test.want1, got1) || !reflect.DeepEqual(test.want2, got2) {
				t.Errorf("%s error\nwant: %+v, %v\ngot: %+v, %v\n", t.Name(), test.want1, test.want2, got1, got2)
			}
		})
