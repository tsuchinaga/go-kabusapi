...
func Test_NewPositionsRequester(t *testing.T) {
	...
	want := &positionsRequester{client: client{token: "token", url: "http://localhost:18080/kabusapi/positions"}}
	got := NewPositionsRequester("token")
			...
			want1: &PositionsResponse{
				{
					...
					ExchangeName:    "東証１部",
					ExecutionDay:    YmdNUM{time.Date(2020, 7, 2, 0, 0, 0, 0, time.Local)},
					Price:           704,
					LeavesQty:       500,
					HoldQty:         0,
					Side:            SideSell,
					...
					Commission:      1620,
					CommissionTax:   162,
					ExpireDay:       YmdNUM{time.Date(2020, 12, 29, 0, 0, 0, 0, time.Local)},
