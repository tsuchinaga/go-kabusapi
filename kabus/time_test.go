...
func Test_HmString_MarshalJSON(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "正常な時刻をパースできる",
			time: HmString{Time: time.Date(0, 1, 1, 13, 57, 0, 0, time.Local)},
			want: []byte(`13:57`)},
	}
...
func Test_HmString_UnmarshalJSON(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "正常系のパース", src: []byte(`"13:56"`),
			want: HmString{Time: time.Date(0, 1, 1, 13, 56, 0, 0, time.Local)}},
		{name: "nullはゼロ値にする", src: []byte(`null`), want: HmString{}},
		{name: "空文字はゼロ値にする", src: []byte(`""`), want: HmString{}},
		{name: "nilはゼロ値にする", src: nil, want: HmString{}},
	}
...
func Test_HmsString_MarshalJSON(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "正常な時刻をパースできる",
			time: HmsString{Time: time.Date(0, 1, 1, 13, 57, 26, 0, time.Local)},
			want: []byte(`13:57:26`)},
	}
...
func Test_HmsString_UnmarshalJSON(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "正常系のパース", src: []byte(`"13:56:47"`),
			want: HmsString{Time: time.Date(0, 1, 1, 13, 56, 47, 0, time.Local)}},
		{name: "nullはゼロ値にする", src: []byte(`null`), want: HmsString{}},
		{name: "空文字はゼロ値にする", src: []byte(`""`), want: HmsString{}},
		{name: "nilはゼロ値にする", src: nil, want: HmsString{}},
	}
...
func Test_YmdHmString_MarshalJSON(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "正常な時刻をパースできる",
			time: YmdHmString{Time: time.Date(2021, 4, 22, 13, 57, 0, 0, time.Local)},
			want: []byte(`2021/04/22 13:57`)},
	}
...
func Test_YmdHmString_UnmarshalJSON(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "正常系のパース", src: []byte(`"2021/04/02 03:56"`),
			want: YmdHmString{Time: time.Date(2021, 4, 2, 3, 56, 0, 0, time.Local)}},
		{name: "nullはゼロ値にする", src: []byte(`null`), want: YmdHmString{}},
		{name: "空文字はゼロ値にする", src: []byte(`""`), want: YmdHmString{}},
		{name: "nilはゼロ値にする", src: nil, want: YmdHmString{}},
	}
